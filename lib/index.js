...
/*
 * Default constructor for the library, invoke with:
 * new FJQ(options)
 * where options is an object of parameters, see README for details
 */
var FJQ = function FJQ(options) {
  ...
  log("Initialized new featureless job queue on key " + options.redisKey);
};


/**
 * Process tasks in specified queueWorker function,
 * running up to concurrency (int) tasks at the same time
 */
...
/**
 * Intended for internal use only, but can be used if you need to run custom redis command.
 */
...
/**
 * Add the job / jobs to the job queue, and invoke cb() when *all* tasks have been saved
 * O(n), although jobs are batched 200 at a time by default, so very likely to be O(1) for most use cases
 */
...
/**
 * Remove all existings jobs from this queue
 * O(n)
 */
...
/**
 * Returns the number of jobs currently stored in the job queue
 * O(1)
 */
...
/**
 * Shutdown the queue, finish all the current workers jobs and exit properly.
 * Closes the Redis connection, or flush the blocked one.
 */
